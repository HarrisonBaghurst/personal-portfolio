    ...
    return (
        <AccordionPrimitive.Header className="flex">
            <AccordionPrimitive.Trigger
                ...
                className={cn(
                    "group/accordion-trigger relative flex flex-1 cursor-pointer items-start justify-between gap-4 rounded-sm border border-transparent py-5 text-left text-heading transition-all outline-none not-has-[:is(h1,h2,h3,h4)]:hover:underline hover:[&_:is(h1,h2,h3,h4)]:underline focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 **:data-[slot=accordion-trigger-icon]:ml-auto **:data-[slot=accordion-trigger-icon]:size-6 **:data-[slot=accordion-trigger-icon]:text-foreground/50 mt-8",
                    className,
                )}
                {...props}
            >
...
function AccordionContent({
    ...
    return (
        ...
        >
            <div
                className={cn(
                    "flex h-(--radix-accordion-content-height) flex-col gap-4 pt-0 pb-6 [&_a]:text-link",
                    className,
                )}
            >
                {children}
